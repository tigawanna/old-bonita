...
export async function fetchNextjsTanstackTemplates(){
    try {
        const package_json = await safeJSONParse<IPackageJson>(await getTanstckNextTemplateFile("package.json"))
...
export async function updateNextjsPkgJson(templates:Awaited<ReturnType<typeof fetchNextjsTanstackTemplates>>){
...
export async function updateNextJsfilesWithTemplates(template:UpdateNextTemplates,bonita_config:TBonitaConfigSchema){
    ...
    try {
        ...
        for await (const key of Object.keys(template)) {
            // if (!template[key as keyof UpdateNextTemplates]) {
                //     throw new Error("template missing key")
                // }
            if (key === "package.json") {
                    printHelpers.warning("skipping")
                    continue
            }

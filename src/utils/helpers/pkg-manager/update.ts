...
type DevDeps ={[key: string]: string}
type MainDeps ={[key: string]: string}
...
export async function promiseAllSettled(deps_arr: Promise<any>[], name:string){
...
export async function updateInnerDeps(pkg_grp:string,deps_json:InnerDeps){
  const spinnies = new Spinnies()
  try {
    spinnies.add("main", { text: `checking ${name} deps` })
      const deps_arr = Object.entries(deps_json).map(([name, version]) => {
        console.log(pkg_grp,name, version)
        return { pkg_grp, name, version }

...
export async function updateTWOptions(options:Omit<Config,"plugins">) {
    try {
        ...
        const default_config = (await getDefaultExportOptions(mod)) as Config;
        ...
        printHelpers.success("tailwind boilerpate setup succefully successfully",code);
...
export async function updateTWPlugins(plugins: {name:string,config?:{[key:string]:string}}[]) {
    try {
        const tw_path = pathExists(await fg(["tailwind.config.*"], {}));
        if (!tw_path) {
            return "no tailwind config found";
        ...
        if(plugin.config){
            return `require('${plugin.name}')(${JSON.stringify(plugin.config, null, 2)})`;
        }
        return `require('${plugin.name}')`;
        })
        
        mod.exports.default.plugins = [...mod.exports.default.plugins,...tw_plugins];
